<template>
	<div>  <!-- 必须多套一层，不然背景颜色会出现问题 -->
		<div class="wrapper">
			...
			<ra-card>
				...
				<div class="content">
					<div class="content-head">
						...
						<!-- 这里有BUG -->
						<p v-if="channel.channelInfo">有效期至：<span class="text-bold">{{ channel.channelInfo.expireTime | dateFormat('YYYY-MM-DD') }}</span></p>
			...
			<ra-card class="pay-detail">
				<p slot="title">
					频道消费
					<a v-show="order.total > order.limits" class="link" href="#">查看更多</a>
				</p>
				<div class="content">
					<!-- 表格 -->
					<el-table :data="order.data" border v-loading="loading" class="pay-detail__table">
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions('account',{
				orderFind: 'ACCOUNT_ORDER_FIND',
				payOrder: 'ACCOUNT_ORDER_PAY'
			}),
			...mapActions('user',{
				getUser: 'USER_GET'
			}),
			...
			selectPage (page) {
				this.loading = true;
				this.order.page = page || this.order.page;
				this.orderFind(this.order).then(res => {
					this.order.data = res.data.orders;

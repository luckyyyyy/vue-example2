<template>
	<el-dropdown
		v-if="!members[account] || members[account] && members[account].type != 'owner'"
		trigger="click"
	...
	>
		...
		<el-dropdown-menu slot="dropdown">
			...
			<template v-if="!members[account]">
				<el-dropdown-item @click.native="setUser('manager', true)">设为管理员</el-dropdown-item>
				<el-dropdown-item @click.native="setUser('common', true)">设为会员</el-dropdown-item>
				<el-dropdown-item @click.native="setUser('gag', true)" divided>禁言</el-dropdown-item>
			</template>
			<template v-else>
				...
				<template v-else-if="members[account].type == 'manager'">
					<el-dropdown-item @click.native="setUser('manager', false)">取消管理员</el-dropdown-item>
				</template>
				<template v-if="members[account].type != 'manager' && members[account].type != 'common'">
					<el-dropdown-item @click.native="setUser('common', true)">设为会员</el-dropdown-item>
				</template>
				<template v-else-if="members[account].type == 'common'">
					<el-dropdown-item @click.native="setUser('common', false)">取消会员</el-dropdown-item>
				</template>
				<template v-if="members[account].type != 'restricted'">
					<el-dropdown-item @click.native="setUser('gag', true)" divided>禁言</el-dropdown-item>
				</template>
				<template v-else-if="members[account].type == 'restricted'">
					<el-dropdown-item @click.native="setUser('gag', false)" divided>解除禁言</el-dropdown-item>
				</template>
			</template>
		</el-dropdown-menu>
	</el-dropdown>

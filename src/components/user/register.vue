	...
	<div class="form">
		...
		<el-form :rules="rules" ref="sign_up" label-width="100px" :model="sign_up" v-loading.body="lock" element-loading-text="拼命加载中">
			<el-form-item label="手机号码" prop="username">
				<el-input v-model="sign_up.username" placeholder="请输手机号码"></el-input>
			</el-form-item>
			<el-form-item class="smscode" label="短信验证码" prop="smscode">
				<el-input v-model="sign_up.smscode" placeholder="请输入短信验证码"></el-input>
				<el-button :disabled="countdown && true" @click="sign_up_smscode">{{ interval ? countdown : '获取验证码' }}</el-button>
			</el-form-item>
...
export default {
	...
	methods: {
		sign_up_smscode () {
			this.$refs.sign_up.validateField('username', errors => {
				if (!errors) {
					const phone = this.sign_up.username;
					this.$store.dispatch('SIGNUP_SMSCODE_REQUEST', { phone }).then( res => {
						...
						if (err.data.code == 20200) {
							...
							this.$alert(_L[err.data.code], '错误', {
								type: 'error'
							})
						}
			...
			this.$refs.sign_up.validate((valid) => {
				if (valid) {
					this.$store.dispatch('SIGNUP_REQUEST', this.sign_up).then( res => {
						this.$store.commit('SIGNUP_SUCCESS', res.data);
